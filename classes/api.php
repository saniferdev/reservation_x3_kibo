...
Class API{
    ...
    public function getId($id){
        ...
        $query          = "SELECT commande FROM [COMMANDE_KIBO].[dbo].[commande_en_ligne] WHERE commande = ".$id." ";
    ...
    public function insertId($id){
        $query  = "INSERT INTO [COMMANDE_KIBO].[dbo].[commande_en_ligne] (commande) VALUES (".$id.") ";
    ...
    public function deleteCommandeId($id){
        $query  = "DELETE FROM [COMMANDE_KIBO].[dbo].[commande_en_ligne] WHERE commande = '".$id."' ";
        if(sqlsrv_query($this->link, $query)) return true;
        else return var_dump(sqlsrv_errors());
    }

    public function entete($client,$pro_num,$adresse,$tel,$email,$id,$carrier){
        $mode_paiement  =   $this->getOrderPaiement($id);
        $reference      =   $this->getReference($id);
        $compte_pro     =   (!empty($pro_num)) ? $pro_num : 'P01K0001';
        $carriers       =   (isset($carrier) && !empty($carrier)) ? $carrier : 'Retrait KIBO (DRIVE)';
        ...
        $array2         =   array(
                                'Retrait KIBO (DRIVE)' => 'KIB',
                                'Livraison en Province' => 'PRO',
                                'Livraison Antananarivo' => 'ANT'
        ...
        $array3         =   array(
                                ...
                                'MVola' => 'NVO',
                                'Orangemoney' => 'ORA',
                                'Paiement en magasin' => 'ESP',
                                'Paiement par carte VISA / MASTERCARD' => 'CB',
                                'Paiement par carte VISA / MASTER' => 'CB',
                                'mise_en_compte' => 'ESP',
                                'Carte de crédit' => 'CB',
                                'Paiement par carte VISA / MASTERCARD : Visa' => 'CB',
                                'en_especes' => 'ESP',
                                'En especes au livreur' => 'ESP',
                                'CybersourceOfficial' => 'VIR',
                                'Transfert bancaire' => 'VIR'
                            );


        $YPTE           =   $array[$mode_paiement];
        $info_liv       =   $array2[$carriers];
        $info_pay       =   $array3[$mode_paiement];

        $xml            =   '<GRP ID="IN">
                                <FLD NAM="YSALFCY">KIB01</FLD>
                                <FLD NAM="YSTOFCY">KIB01</FLD>
                                <FLD NAM="YBPCORD">'.$compte_pro.'</FLD>
                                <FLD NAM="YPTE">'.$YPTE.'</FLD>
                                <FLD NAM="YBPCNAM1">'.$reference.' - '.$client.'</FLD>
                                ...
                                <FLD NAM="YYINFLIV">'.$info_liv.'</FLD>
                                <FLD NAM="YYMOYPAY">'.$info_pay.'</FLD>
                                <FLD NAM="YBPINAM">'.$client.'</FLD>
                                <FLD NAM="YBPIADDLIG">'.$adresse.'</FLD>
                                <FLD NAM="YBPIADDLIG2">'.$email.'</FLD>
                                <FLD NAM="YBPIADDLIG3">'.$tel.'</FLD>
                                <FLD NAM="YBPIPOSCOD"></FLD>
                                <FLD NAM="YBPICTY"></FLD>
                              </GRP>';
        ...
        while ($iterator->valid()) {
            if ($iterator->hasChildren()) {
                foreach ($iterator->getChildren() as $value) {
                    if(!empty($value["pro_num"][0])) $pro_num = $value["pro_num"][0];
                    $client = strtoupper($value["lastname"][0]).'  '.strtoupper($value["firstname"][0]).'-'.$value["email"][0].'-'.$pro_num;
                }
            }
            $iterator->next();
        }
    ...
    public function getClientAdress($id){
        ...
        while ($iterator->valid()) {
            if ($iterator->hasChildren()) {
                foreach ($iterator->getChildren() as $value) {
                    if($value["address1"][0]) $ad = $value["address1"][0];
                    if($value["phone"][0]) $ph = $value["phone"][0];
                    $client = strtoupper($ad).'-'.$ph;
                }
            }
            $iterator->next();
        }
    ...
    public function getOrderCarriers($id){
        $xml        = file_get_contents($this->url."/api/carriers/".$id."/?ws_key=".$this->key);
        $getContent = $this->xml2array($xml);
        $iterator   = new RecursiveArrayIterator($getContent);
        $carriers   = "";
        while ($iterator->valid()) {
            if ($iterator->hasChildren()) {
                foreach ($iterator->getChildren() as $value) {
                    if($value["name"][0]) $carriers = $value["name"][0];
                }
            }
            $iterator->next();
        }
        ...
        while ($iterator->valid()) {
            if ($iterator->hasChildren()) {
                foreach ($iterator->getChildren() as $value) {
                    ...
                    $carriers = $this->getOrderCarriers($value["id_carrier"][0]);
                    foreach(end($value["associations"]) as $val){
                        foreach($val[0]["order_row"] as $key => $valeur){
                            $array[]['article'] = $valeur['product_reference'][0].'-'.$valeur['product_quantity'][0].'-'.$client.'-'.$adresse.'-'.$carriers;
                        }
                    }
                }
            }
                    ...
                    if( $id_commande == 0 ){
                        if($this->insertId($value["id"]) == TRUE){
                            ...
                            foreach ($detail as $key => $article) {
                                        if(isset($article['article'])){
                                            ...
                                            if($i == 0){
                                                ...
                                                $entete   = $this->entete($exp[2],$exp[4],$exp[5],$exp[6],$exp[3],$value["id"],$exp[7]);
                                                $ligne    = '<TAB ID="IND">';
                                                $pro_num  = $exp[4];
                                                $xml_Ref .= '<TAB ID="IND">';
                                            ...
                                            /*$qteRetour  = $this->soap_API('YPANIER2',$xml_Ref);
                            ...
                            if(isset($entete) && isset($ligne)){
                                ...
                                if( isset($commande) ) {
                                    //$inn = $this->insertId($value["id"]);
                                    //if($this->insertId($value["id"]) == true){
                                        
                                        if($pro_num == ""){
                                            $compte = 'Client Divers';
                                        }
                                        else{
                                            $compte = 'Client Pro';
                                        }
                                        $adresse = "Commande.Kibo@kibo.mg";
                                        
                                        $sujet  = "Commande validée sur KIBO et crée dans X3";
                                        $objet  = "Bonjour,<br><br>";
                                        $objet .= "Une commande n° ".$value["id"]." a été crée sur le site WEB de KIBO.<br>";
                                        $objet .= "La commande sur le site KIBO a été crée dans sage X3.<br><br>";
                                        $objet .= "Ci-après les details de la commande:<br>";
                                        $objet .= "<strong>N° :</strong>".$commande." - <strong>".$compte." :</strong> ".$exp[2]."<br><br>";
                                        $objet .= $e."<br><br>";
